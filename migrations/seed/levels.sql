...
--
-- KONWENCJA „X+" (górne tiery open-ended w źródle, np. frontend L4 „250+") — D22:
--   hours_max = 1.5 × hours_min (DRAFT, per-obszar do korekty Jakuba). Realny nadmiar
--   ponad górną granicę wypełnia override_hours w wycenie z uzasadnieniem.
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='frontend'), 4, 'Enterprise / Custom', 'Pełny design system, mikrointerakcje, multi-brand, white-label, WCAG AAA', 250, 375),
  ...
  ((SELECT id FROM est_aspects WHERE code='apis'), 4, 'Enterprise / Custom', 'gRPC / architektura mikroserwisowa, event-driven, API gateway, kontrakty SLA', 160, 240),
  ...
  ((SELECT id FROM est_aspects WHERE code='emails'), 4, 'Enterprise / Custom', 'Własna infrastruktura wysyłki, wielojęzyczne szablony, compliance', 32, 48)
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='backend_logic'), 4, 'Enterprise / Custom', 'Wielomodułowy system, złożone przepływy, integracje z ERP/WMS/PIM', 250, 375),
  ...
  ((SELECT id FROM est_aspects WHERE code='database'), 4, 'Enterprise / Custom', 'Rozproszona architektura danych, multi-region, CQRS / event sourcing', 160, 240),
  ...
  ((SELECT id FROM est_aspects WHERE code='storage'), 4, 'Enterprise / Custom', 'Storage rozproszony, retencja/compliance, replikacja DR', 32, 48),
  ...
  ((SELECT id FROM est_aspects WHERE code='caching'), 4, 'Enterprise / Custom', 'Cache rozproszony, cache warming, złożone strategie invalidacji', 64, 96)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='authentication'), 4, 'Enterprise / Custom', 'SSO / SAML enterprise, zaawansowane compliance i audyt logowań', 96, 144),
  ...
  ((SELECT id FROM est_aspects WHERE code='permissions'), 4, 'Enterprise / Custom', 'Złożony model uprawnień, delegacja, pełny audyt zmian uprawnień', 80, 120),
  ...
  ((SELECT id FROM est_aspects WHERE code='security'), 4, 'Enterprise / Custom', 'Certyfikacje (ISO 27001/SOC2/PCI-DSS), regularne pentesty, bug bounty', 96, 144),
  ...
  ((SELECT id FROM est_aspects WHERE code='rls'), 4, 'Enterprise / Custom', 'Złożone polityki dynamiczne, pełny audyt dostępu do danych', 64, 96)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='infrastructure'), 4, 'Enterprise / Custom', 'Multi-cloud/hybrid, pełne IaC, hosting wieloregionalny', 80, 120),
  ...
  ((SELECT id FROM est_aspects WHERE code='cdn'), 4, 'Enterprise / Custom', 'Multi-CDN z failover, edge computing/functions', 32, 48),
  ...
  ((SELECT id FROM est_aspects WHERE code='load_balancing'), 4, 'Enterprise / Custom', 'Globalny LB, multi-region active-active', 48, 72),
  ...
  ((SELECT id FROM est_aspects WHERE code='rate_limiting'), 4, 'Enterprise / Custom', 'Distributed rate limiting, polityki per klient/tier', 32, 48),
  ...
  ((SELECT id FROM est_aspects WHERE code='high_availability'), 4, 'Enterprise / Custom', 'SLA 99,99%+, multi-region active-active', 80, 120),
  ...
  ((SELECT id FROM est_aspects WHERE code='disaster_recovery'), 4, 'Enterprise / Custom', 'Pełny DR z regularnym testowaniem, RPO < 1h', 80, 120)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='cicd'), 4, 'Enterprise / Custom', 'Multi-środowiskowy pipeline, automatyczne rollbacki, compliance gates', 64, 96),
  ...
  ((SELECT id FROM est_aspects WHERE code='observability'), 4, 'Enterprise / Custom', 'Pełna observability (traces/metrics/logs), retencja compliance, SIEM, on-call', 56, 84)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='analytics'), 4, 'Enterprise / Custom', 'CDP (Customer Data Platform), atrybucja multi-touch, integracja z BI', 48, 72),
  ...
  ((SELECT id FROM est_aspects WHERE code='sst'), 4, 'Enterprise / Custom', 'Własna infrastruktura server-side, zaawansowana walidacja i wzbogacanie danych', 80, 120),
  ...
  ((SELECT id FROM est_aspects WHERE code='seo'), 4, 'Enterprise / Custom', 'Wielorynkowe SEO, programmatic SEO, złożona architektura informacji', 64, 96),
  ...
  ((SELECT id FROM est_aspects WHERE code='sem'), 4, 'Enterprise / Custom', 'Setup multi-market feed management (bez prowadzenia — abonament)', 48, 72),
  ...
  ((SELECT id FROM est_aspects WHERE code='geo'), 4, 'Enterprise / Custom', 'Setup optymalizacji dla wielu silników AI, framework testów treści (prowadzenie — abonament)', 48, 72),
  ...
  ((SELECT id FROM est_aspects WHERE code='consent'), 4, 'Enterprise / Custom', 'Consent management dla wielu marek/domen, pełny audyt compliance', 32, 48)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
...
INSERT INTO est_levels (aspect_id, level, name, description, hours_min, hours_max) VALUES
  ...
  ((SELECT id FROM est_aspects WHERE code='discovery'), 4, 'Enterprise / Custom', 'Warsztaty wielodniowe, analiza konkurencji, pełna specyfikacja z architekturą', 40, 60),
  ...
  ((SELECT id FROM est_aspects WHERE code='uxui'), 4, 'Enterprise / Custom', 'Od zera: identyfikacja wizualna + design system + pełny projekt', 100, 150),
  ...
  ((SELECT id FROM est_aspects WHERE code='qa'), 4, 'Enterprise / Custom', 'Pełny plan testów, automatyzacja, testy wydajnościowe/obciążeniowe', 64, 96),
  ...
  ((SELECT id FROM est_aspects WHERE code='data_migration'), 4, 'Enterprise / Custom', 'Setki tysięcy rekordów, dane wrażliwe/szyfrowane, migracja etapowa z oknem przełączenia', 100, 150),
  ...
  ((SELECT id FROM est_aspects WHERE code='content'), 4, 'Enterprise / Custom', 'Masowy content: >1000 pozycji, wielojęzyczność, redakcja/adaptacja treści', 64, 96),
  ...
  ((SELECT id FROM est_aspects WHERE code='golive'), 4, 'Enterprise / Custom', 'Wdrożenie etapowe/blue-green, plan rollbacku, dedykowany dyżur', 40, 60)
ON CONFLICT(aspect_id, level) DO UPDATE SET
  name = excluded.name, description = excluded.description,
  hours_min = excluded.hours_min, hours_max = excluded.hours_max;
