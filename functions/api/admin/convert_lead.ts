...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // 1. Get Lead
    const lead: any = await env.DB.prepare('SELECT * FROM leads WHERE id = ?').bind(lead_id).first();
    if (!lead) return new Response('Lead not found', { status: 404 });

    // 2. Check if user already exists
    let user: any = await env.DB.prepare('SELECT id FROM users WHERE email = ?').bind(lead.email).first();
    ...
    if (!user) {
      ...
      const result: any = await env.DB.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `).bind(lead.email, lead.name, lead.website || '', 'client', 1).run();
      
      // Get the newly created ID
      const newUser: any = await env.DB.prepare('SELECT id FROM users WHERE email = ?').bind(lead.email).first();
      user = { id: newUser.id };
    }

    // 3. Update Lead status
    await env.DB.prepare('UPDATE leads SET status = ?, user_id = ? WHERE id = ?')
      .bind('converted', user.id, lead_id).run();
