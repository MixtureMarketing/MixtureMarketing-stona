...
const getIconForTech = (name: string) => {
  ...
  if (normalized.includes('cms') || normalized.includes('word') || normalized.includes('sanity')) return <Database size={16} />;
...
const CaseStudyTemplate = () => {
  ...
  const [activeAsset, setActiveAsset] = useState<{ asset: SanityImage; caption?: string } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-dark selection:bg-primary selection:text-white">
        ...
        {activeAsset && (
          ...
          >
            <div className="min-h-screen w-full flex flex-col items-center justify-center py-10 px-4">
                ...
                <p className="mt-8 text-white/50 text-xs font-bold uppercase tracking-[0.2em]">Przewiń aby zobaczyć całość</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <div className="sticky top-28 space-y-8">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-xl shadow-gray-200/50 border border-gray-100">
                        ...
                        {isWeb && (
                            <div className="space-y-8">
                                {project.websiteUrl && (
                                    <a href={project.websiteUrl} target="_blank" rel="noopener noreferrer" className="group relative block w-full overflow-hidden rounded-xl bg-dark text-white p-4 text-center font-bold hover:shadow-lg hover:shadow-blue-900/20 transition-all">
                                    ...
                                    <div className="bg-gray-50 rounded-2xl p-4 flex items-center gap-4">
                                        <div className="relative w-16 h-16 flex items-center justify-center">
                                            <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                                                ...
                                                <path className="text-green-500 drop-shadow-sm" strokeDasharray={`${project.performanceScore}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="3" />
                                            </svg>
                                            <span className="absolute text-lg font-black text-green-600">{project.performanceScore}</span>
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {project.techStack.map(tech => (
                                                <span key={tech} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold border border-blue-100">
                                    ...
                                    <div>
                                         ...
                                         <div className="flex flex-wrap gap-2">
                                            {project.integrations.map(tech => (
                                                <span key={tech} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold border border-emerald-100">
                                                    {getIconForTech(tech)} {tech}
                                                </span>
                                            ))}
                                        </div>
                             ...
                             <div className="space-y-6">
                                {project.tools && (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {project.tools.map(t => (
                                                <span key={t} className="px-3 py-1.5 bg-purple-50 text-purple-700 rounded-lg text-xs font-bold border border-purple-100 flex items-center gap-1.5">
                                                    <PenTool size={14} /> {t}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                {project.downloads && (
                                    <div className="pt-6 border-t border-gray-100">
                                        <p className="text-xs font-bold text-gray-400 uppercase mb-3">Pliki do pobrania</p>
                                        <div className="space-y-3">
                                            {project.downloads.map((file, i) => (
                                                <a key={i} href={file.asset.url} target="_blank" className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors group">
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {project.platforms.map(p => (
                                        <span key={p} className="px-3 py-1.5 bg-orange-50 text-orange-700 rounded-lg text-xs font-bold border border-orange-100 flex items-center gap-1.5">
                                            <Globe size={14} /> {p}
                                        </span>
                                    ))}
                                </div>
                    ...
                    {project.credits && project.credits.length > 0 && (
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                             <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-6">Zespół Projektowy</h4>
                    ...
                    <AuditTeaser layout="compact" variant="light" buttonText="Analizuj stronę" placeholder="Wpisz adres www..." />
                </div>
            </aside>
        </div>
      </main>
      ...
      {nextProject && (
         <section className="bg-dark py-24 relative overflow-hidden group cursor-pointer" onClick={() => navigate(`/portfolio/${nextProject.slug}`)}>
            {/* Bg Image Overlay */}
            {nextProject.mainImage && (
                <div className="absolute inset-0 opacity-20 group-hover:opacity-10 transition-opacity duration-700">
                    <img src={urlFor(nextProject.mainImage).width(1920).url()} alt="" className="w-full h-full object-cover grayscale mix-blend-multiply" />
                </div>
            )}
            
            <div className="container mx-auto px-4 relative z-10 text-center">
                <p className="text-white/60 text-sm font-bold uppercase tracking-[0.3em] mb-4">Następny Projekt</p>
