...
const TopicLink: React.FC<TopicLinkProps> = ({ icon, title, desc, target, status }) => (
  <Link
    ...
    className={`group p-6 rounded-2xl border transition-all flex items-start gap-4 ${status === 'ready' ? 'bg-gray-50 border-gray-100 hover:border-primary hover:bg-white hover:shadow-xl' : 'bg-gray-50/50 border-dashed border-gray-200 opacity-80 cursor-not-allowed'}`}
  >
    <div
