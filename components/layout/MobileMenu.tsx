...
const MobileMenu: React.FC<MobileMenuProps> = React.memo(
  ({
    ...
    useFocusTrap(menuRef, isOpen, closeMenu);

    return (
      <div
        id="mobile-menu"
        ref={menuRef}
        className={`lg:hidden fixed inset-0 z-[var(--z-header)] bg-white/98 backdrop-blur-2xl transform transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? 'translate-x-0 visible' : 'translate-x-full invisible'} flex flex-col h-dvh`}
        aria-hidden={!isOpen}
        role="navigation"
      ...
      >
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        ...
                        {section.items.map((item, itemIdx) => (
                          <Link
                            ...
                            onClick={() => {
                              closeMenu();
                              toggleDropdownMobile(CONTENT.offer.label);
                            }}
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xxs text-gray-700 font-medium leading-tight">
                                {item.desc}
                              </span>
                            </div>
                          </Link>
        ...
        <div
          className={`p-6 pb-safe bg-dark text-white rounded-t-[3rem] shadow-[0_-10px_40px_rgba(33,50,97,0.2)] transition-all duration-700 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}
        >
          <div className="flex items-center justify-between mb-6 px-2">
            <div>
              <p className="text-xxs font-bold text-accent-dark uppercase tracking-[0.2em] mb-1 opacity-80">
