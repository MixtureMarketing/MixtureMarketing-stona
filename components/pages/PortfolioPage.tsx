...
const PortfolioPage = () => {
  ...
  const [filteredProjects, setFilteredProjects] = useState<SanityCaseStudy[]>([]);
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                >
                    ...
                    <p className="text-xl text-gray-600 max-w-2xl leading-relaxed">
                        Od zaawansowanych aplikacji webowych, przez skuteczne kampanie marketingowe, po unikalny branding. Poznaj projekty, które przynoszą realne wyniki.
                    </p>
                </motion.div>
                        ...
                        return (
                            <button
                                ...
                                className={`relative px-6 py-3 rounded-full text-sm font-bold transition-all flex items-center gap-2 group overflow-hidden ${
                                    isActive ? 'text-white' : 'text-gray-500 hover:text-dark bg-white border border-gray-200'
                                }`}
                            >
                                {isActive && (
                                    <motion.div
                                ...
                                <span className="relative z-10 flex items-center gap-2">
                                    <Icon size={16} className={isActive ? 'text-primary' : 'text-gray-400 group-hover:text-primary transition-colors'} />
                                    {cat.label}
                                </span>
                            </button>
                        );
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 pb-32">
            {loading ? (
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="h-[500px] bg-white rounded-[2rem] border border-gray-100 shadow-sm animate-pulse flex flex-col overflow-hidden">
                                    ...
                                    <div className="relative h-72 overflow-hidden bg-gray-100">
                                        {project.mainImage ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-gray-400">
                                                <span className="font-bold uppercase tracking-widest text-xs">Brak zdjęcia</span>
                                            </div>
                                        )}
                                        
                                        {/* Overlay Tags */}
                                        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                                            <span className={`px-3 py-1 text-xxs font-bold uppercase tracking-wider rounded-lg backdrop-blur-md shadow-sm border border-white/20 text-white ${
                                                project.category === 'web' ? 'bg-blue-600/90' : 
                                                project.category === 'marketing' ? 'bg-indigo-600/90' : 'bg-purple-600/90'
                                            }`}>
                                                {project.category}
                                            </span>
                                        </div>
                                        ...
                                        {project.subcategory && (
                                            <div className="flex flex-wrap gap-2 pt-6 border-t border-gray-100">
                                                {project.subcategory.slice(0, 3).map((sub, i) => (
                                                    <span key={i} className="text-xxs font-bold uppercase text-gray-500 bg-gray-50 px-2 py-1 rounded border border-gray-100">
                                                        {sub}
                                                    </span>
                                                ))}
                                                {project.subcategory.length > 3 && (
                                                    <span className="text-xxs font-bold text-gray-400 px-1 py-1">+ {project.subcategory.length - 3}</span>
                                                )}
                                            </div>
                                        )}
                                    </div>
            ...
            {!loading && filteredProjects.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[3rem] shadow-sm border border-gray-100">
                    ...
                    <p className="text-gray-500 max-w-md mx-auto mb-8">
                        W tej kategorii nie mamy jeszcze opublikowanych case studies. Sprawdź inne kategorie lub wróć do nas wkrótce.
                    </p>
                    <Button
                        variant="primary"
