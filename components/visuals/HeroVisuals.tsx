    ...
    <AnimateOnScroll className="relative z-10 flex justify-center lg:justify-end">
      <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-gray-100 p-6 md:p-10 w-full max-w-[320px] md:max-w-none relative overflow-hidden group hover:shadow-[0_40px_80px_-20px_rgba(63,61,145,0.15)] transition-all duration-500">
        ...
        <div className="flex justify-center mb-6 md:mb-10 relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`text-4xl md:text-6xl font-black tracking-tighter leading-none ${perfScore > 90 ? 'text-dark' : 'text-[#F4B400]'}`}>
              {perfScore}
            </span>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <div className="p-3 md:p-4 bg-gray-50 rounded-xl md:rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
            <div className="text-xxxs md:text-xxs font-bold text-gray-600 uppercase mb-1">SEO Readiness</div>
          ...
          <div className="p-3 md:p-4 bg-gray-50 rounded-xl md:rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
            <div className="text-xxxs md:text-xxs font-bold text-gray-600 uppercase mb-1">Accessibility</div>
      ...
      <div className="absolute -bottom-4 -left-4 md:-bottom-8 md:-left-8 bg-white p-3 md:p-4 rounded-xl md:rounded-2xl shadow-xl border border-gray-100 animate-float z-20">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div>
            <div className="text-xxxs md:text-xxs font-bold text-gray-600 uppercase leading-none mb-1">LCP Speed</div>
            <div className="text-xs md:text-sm font-black text-dark">0.7s</div>
          </div>
        </div>
      </div>
      <style>{`
        @keyframes scan { 0% { transform: translateY(-100%); opacity: 0; } 50% { opacity: 1; } 100% { transform: translateY(400%); opacity: 0; } }
...
export const UiUxHeroVisual: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[600px] flex items-center justify-center perspective-[2000px]">
      <div
    ...
    <div className="relative z-10 bg-[#0F172A]/90 backdrop-blur-xl rounded-2xl shadow-[0_0_50px_rgba(97,182,222,0.15)] border border-[#1E293B] transform rotate-1 hover:rotate-0 transition-all duration-700 group overflow-hidden">
      ...
      <div className="p-6 bg-[#0B1120]">
        <div className="flex justify-between items-end mb-6">
          <div>
            <div className="text-xs text-gray-300 uppercase font-bold tracking-wider mb-1">Monthly Revenue</div>
            <div className="text-3xl font-black text-white flex items-center gap-2">
              {(124500 + counter * 125).toLocaleString()} PLN
              <span className="text-xs font-bold text-success bg-success/10 px-2 py-0.5 rounded border border-success/20">+12%</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={i} className="bg-[#1E293B] p-3 rounded-lg border border-[#334155]">
              <div className="text-xxs text-gray-300 uppercase mb-1 font-bold">{s.l}</div>
              <div className="text-sm font-bold text-white flex items-center gap-1">{s.v} <TrendingUp size={10} className="text-success" /></div>
            </div>
          ))}
        </div>
      </div>
      <style>{`
        @keyframes scan-line { 0% { left: 0%; } 100% { left: 100%; } }
...
export const CorporateHeroVisual: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-lg aspect-square flex items-center justify-center transition-transform duration-200 ease-out will-change-transform"
      style={{ transform: `translate3d(${mousePos.x * 20}px, ${mousePos.y * 20}px, 0) rotateX(${mousePos.y * -5}deg) rotateY(${mousePos.x * 5}deg)` }}
    >
      <div className="absolute inset-0 rounded-full border border-dashed border-gray-200 animate-spin-slow-corp opacity-40"></div>
      <svg className="w-full h-full relative z-10 drop-shadow-[0_0_30px_rgba(63,61,145,0.15)]" viewBox="0 0 400 400">
        <g opacity="0.4">
          <path d="M 200 200 L 80 80 M 200 200 L 320 80 M 200 200 L 80 320 M 200 200 L 320 320" stroke="#61B6DE" strokeWidth="1" fill="none" />
        </g>
        {[
          { x: 200, y: 60 }, { x: 60, y: 200 }, { x: 340, y: 200 }, { x: 200, y: 340 }
        ].map((node, i) => (
          <g key={i}>
            <circle cx={node.x} cy={node.y} r="5" fill="white" stroke="#3F3D91" strokeWidth="2" />
            <circle cx={node.x} cy={node.y} r="10" fill="#3F3D91" opacity="0.05" className="animate-ping" />
          </g>
        ))}
        <rect x="175" y="175" width="50" height="50" rx="12" fill="#3F3D91" className="animate-pulse" />
        <Server x="188" y="188" size={24} className="text-primary" />
      </svg>
      <div className="absolute top-0 right-0 p-4 bg-white/80 backdrop-blur shadow-xl rounded-2xl border border-gray-100 animate-float"><ShieldCheck size={16} className="text-emerald-500" /></div>
      <style>{`
        @keyframes spin-corp { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .animate-spin-slow-corp { animation: spin-corp 40s linear infinite; }
      `}</style>
    </div>
  );
};
...
export const DesignHeroVisual: React.FC = () => {
  return (
    <div className="relative w-80 h-80 md:w-96 md:h-96">
      ...
      <style>{`
        @keyframes spin-design { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .animate-spin-slow-design { animation: spin-design 30s linear infinite; }
      `}</style>
    </div>
  );
};
...
export const WebAppHeroVisual: React.FC = () => {
  const [randomHeights] = useState(() => Array.from({ length: 20 }).map((_, i) => ((i * 17) % 80) + 10));
  ...
  return (
    <div className="relative z-10 bg-[#0F172A] rounded-xl border border-[#334155] p-6 shadow-2xl overflow-hidden group hover:shadow-[0_0_40px_rgba(97,182,222,0.15)] transition-shadow duration-500">
      <div className="flex justify-between items-center mb-6 border-b border-[#334155] pb-4">
        <div className="text-xs font-mono text-primary flex items-center gap-2"><Activity size={14} className="animate-pulse" /> SYSTEM_MONITOR_V2</div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#1E293B] p-4 rounded-lg border border-[#334155]">
          ...
          <div className="h-1 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-primary w-[75%] animate-pulse"></div></div>
        </div>
        <div className="bg-[#1E293B] p-4 rounded-lg border border-[#334155]">
          ...
          <div className="h-1 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-success w-[34%]"></div></div>
        </div>
        <div className="col-span-2 bg-[#1E293B] p-4 rounded-lg border border-[#334155] relative overflow-hidden h-32 flex items-end">
          <div className="flex items-end justify-between w-full h-20 gap-1">
            {randomHeights.map((h, i) => (
              <div key={i} className="bg-secondary w-full rounded-t opacity-80 transition-all duration-500" style={{ height: `${h}%` }}></div>
            ))}
          </div>
        </div>
      </div>
    ...
    <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden transform rotate-y-6 rotate-x-6 hover:rotate-0 transition-transform duration-700">
      ...
      <div className="p-4 bg-white relative min-h-[380px] flex flex-col gap-3">
        {[1, 2, 3, 4, 5, 6].map((pos) => {
          ...
          return (
            <div key={pos} className={`p-2 transition-all duration-500 ${isHero ? 'bg-success/10 border border-success rounded-xl scale-105 shadow-lg' : 'opacity-40 border-b border-gray-50 last:border-0'}`}>
              <div className="flex items-center gap-2 mb-1">
                <div className={`w-6 h-4 rounded text-[10px] font-bold flex items-center justify-center ${isHero ? 'bg-success text-white' : 'bg-gray-100 text-gray-400'}`}>#{pos}</div>
                {isHero && <CheckCircle2 size={12} className="text-success" />}
              </div>
              <div className={`h-2 rounded mb-1 ${isHero ? 'w-32 bg-secondary' : 'w-24 bg-gray-200'}`}></div>
              <div className={`h-3 rounded ${isHero ? 'w-full bg-primary/20' : 'w-2/3 bg-gray-100'}`}></div>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div ref={containerRef} className="relative w-80 h-[400px] perspective-1000 group">
      {['cyan', 'magenta', 'yellow', 'black'].map((color, i) => (
        <div key={i} className="absolute inset-0 bg-white rounded-xl shadow-lg border border-white/10 transition-all duration-1000 ease-in-out flex items-center justify-center overflow-hidden will-change-transform"
          style={{
            transform: activeLayer >= i
        ...
        >
          <div className="absolute top-4 left-4 text-xs font-bold font-mono opacity-50 uppercase">{color[0]} Channel</div>
          <div className="w-48 h-48 rounded-full border-[20px]" style={{ borderColor: color === 'black' ? '#000' : color === 'yellow' ? '#FF0' : color === 'magenta' ? '#F0F' : '#0FF', opacity: 0.5 }}></div>
        </div>
      ))}
      <div className="absolute inset-0 bg-[#F4B400] rounded-xl shadow-[0_0_50px_rgba(244,180,0,0.3)] flex items-center justify-center overflow-hidden transition-all duration-1000 will-change-transform"
        style={{ transform: `translateZ(30px) translateX(40px) rotateY(-15deg) translate(${mousePos.x * -10}px, ${mousePos.y * -10}px)`, opacity: activeLayer === 4 ? 1 : 0, zIndex: 20 }}
      >
        <div className="text-[#0B1120] text-center"><ShoppingCart size={48} className="mx-auto mb-4" /><h3 className="text-2xl font-bold uppercase tracking-widest">Premium</h3></div>
      </div>
    </div>
  );
};